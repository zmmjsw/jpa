package com.github.zmm.jpa.util;
...
public class ExcelUtil {
	...
	public static <T> List<T> excelToList(InputStream in, String sheetName, Class<T> entityClass,
		...
		try {
			 ...
			 * i=0;i<uniqueFields.length;i++){ int col=colMap.get(uniqueFields[i]);
			 * uniqueCells[i]=sheet.getColumn(col); }
			 * 
			 * //2.从指定列中寻找重复行 for(int i=1;i<realRows;i++){ int nullCols=0; for(int
			 * j=0;j<uniqueFields.length;j++){ String
			 * currentContent=uniqueCells[j][i].getContents(); Cell
			 * sameCell=sheet.findCell(currentContent, uniqueCells[j][i].getColumn(),
			 * uniqueCells[j][i].getRow()+1, uniqueCells[j][i].getColumn(),
			 * uniqueCells[j][realRows-1].getRow(), true); if(sameCell!=null){ nullCols++; }
			 * }
			 * 
			 * if(nullCols==uniqueFields.length){ throw new
			 * ExcelException("Excel中有重复行，请检查"); } }
			 */
		...
		if (field != null) {
			...
			if (String.class == fieldType) {
				...
				if("".equals(fieldValue.toString())) {
					...
					String fei=	fieldValue.toString().substring(0, fieldValue.toString().indexOf("."));
					field.set(o, Integer.parseInt(fei));
				}
			} else if ((Long.TYPE == fieldType) || (Long.class == fieldType)) {
				field.set(o, Long.valueOf(fieldValue.toString()));
			} else if (BigDecimal.class == fieldType) {
				if (fieldValue.toString() == "" || "".equals(fieldValue.toString())) {
					field.set(o, new BigDecimal(0));
				} else {
					field.set(o, new BigDecimal(fieldValue.toString().replace(",","")));
				}
				...
				field.set(o, new SimpleDateFormat("yyyy-MM-dd").parse(data));
			} else {
